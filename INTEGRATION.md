...
CleanroomASL requires **Xcode 7 or higher** to be built, and the resulting binary can be used on **iOS 8.0 and higher**.
...
- **[Manual integration](#manual-integration)** — The `CleanroomASL.xcodeproj` Xcode project file is embedded directly within your project. You then add `CleanroomASL.framework` to the *Embedded Binaries* and *Linked Frameworks and Libraries* sections under the *General* tab for your application target.

- **[Carthage integration](#carthage-integration)** — [Carthage](https://github.com/Carthage/Carthage) is a dependency package manager designed to build frameworks. Once Carthage is installed, to add CleanroomASL to your project using Carthage, you would put the line `github "emaloney/CleanroomASL"` in your `Cartfile` and then issue the command `carthage update`.
...
Once Carthage is done building CleanroomASL, you can execute the following Terminal command to see the frameworks built by Carthage:
